...
export default function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!registerName || !registerEmail || !registerPassword || registerPassword !== registerConfirmPassword) return;
    setSubmitting(true);
  ...
  const handleSocialLogin = async () => {
    const success = await loginWithSocial('google');
    if (success) navigate('/');
  };
          ...
          <TabsContent value="login">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Entre na sua conta para comprar ingressos e acessar seus eventos.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="email">Email</Label>
                    <Input id="email" type="email" placeholder="seu@email.com" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="password">Senha</Label>
                      <Link to="/forgot-password" className="text-sm text-primary hover:underline">Esqueceu?</Link>
                    </div>
                ...
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-background px-2 text-muted-foreground">Entrar com</span>
                  </div>
                </div>
              ...
              <CardFooter className="justify-center">
                <p className="text-sm text-center text-muted-foreground">
                  Novo por aqui? <button className="text-primary hover:underline" onClick={() => setActiveTab('register')}>Crie sua conta</button>
                </p>
              </CardFooter>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="register">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Cadastre-se para comprar ingressos para seus eventos favoritos.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <form onSubmit={handleRegister} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="name">Nome Completo</Label>
                    <Input id="name" placeholder="Seu nome" value={registerName} onChange={(e) => setRegisterName(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
              ...
              <CardFooter className="justify-center">
                <p className="text-sm text-center text-muted-foreground">
                  Já tem uma conta? <button className="text-primary hover:underline" onClick={() => setActiveTab('login')}>Faça login</button>
                </p>
              </CardFooter>
            </Card>
          </TabsContent>
